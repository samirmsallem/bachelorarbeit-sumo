...
def send_messages():
    vehicles = detect_waiting_vehicles()

    if(len(vehicles) > 0):
        for vehicle in vehicles:
            behind_vehicles = find_approaching_behind_vehicles(vehicle)
            if(len(behind_vehicles) > 0):
                com.write(traci.simulation.getTime(), vehicle, behind_vehicles, 'Waiting at intersection')
